...
void ExperimentationStation::drawCylinder(GLUquadricObj* quad, float baseRadius, float topRadius, float height, int selectedDrawStyle, GLuint texture, bool isShadow)
{
    // a) declare quadric pointer
    quad = NULL;
...
void ExperimentationStation::drawCircle(float circleRadius, GLuint texture, bool isShadow)
{
    // 1. Bind the texture
    if (isShadow) {
...
void ExperimentationStation::drawCylinderAlongCurve(float angleBegins, float angleEnds, float circleXRadius, float circleYRadius, float cylinderRadius, int selectedDrawStyle, GLuint texture, bool isShadow)
{
    for (float angle = angleBegins; angle <= angleEnds; angle++) {
        ...
        glPushMatrix();
            ...
            {
            case FILL:
                drawCylinder(quad, cylinderRadius, cylinderRadius, 0.1f, selectedDrawStyle, texture,isShadow);
                break;
            case LINE:
                drawCylinder(quad, cylinderRadius, cylinderRadius, 0.1f, selectedDrawStyle, texture, isShadow);
                break;
...
void ExperimentationStation::calculateShadowMatrix(GLfloat matrix[16], GLfloat ground[4], GLfloat light[4]) {
    ...
    matrix[0] = dot - light[0] * ground[0];
    matrix[4] = 0.0f - light[0] * ground[1];
    matrix[8] = 0.0f - light[0] * ground[2];
    matrix[12] = 0.0f - light[0] * ground[3];

    matrix[1] = 0.0f - light[1] * ground[0];
    matrix[5] = dot - light[1] * ground[1];
    matrix[9] = 0.0f - light[1] * ground[2];
    matrix[13] = 0.0f - light[1] * ground[3];

    matrix[2] = 0.0f - light[2] * ground[0];
    matrix[6] = 0.0f - light[2] * ground[1];
    matrix[10] = dot - light[2] * ground[2];
    matrix[14] = 0.0f - light[2] * ground[3];

    matrix[3] = 0.0f - light[3] * ground[0];
    matrix[7] = 0.0f - light[3] * ground[1];
    matrix[11] = 0.0f - light[3] * ground[2];
    matrix[15] = dot - light[3] * ground[3];
